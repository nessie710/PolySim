...
def simulate_mPNP(Vapp, Vbulk, concentrations, plot_flag):
    ...
    c_bulk_scaled1 = concentrations.c_bulk_scaled1/c_char
    c_bulk_scaled2 = concentrations.c_bulk_scaled2/c_char
    c_surf_scaled1 = concentrations.c_surf_scaled1/c_char
    c_surf_scaled2 = concentrations.c_surf_scaled2/c_char
    ...
    domain = mesh.create_interval(comm=MPI.COMM_WORLD, points=(0.0, L_scaled), nx=10000)
    ...
    W = fem.functionspace(domain, ("CG", 1))  # o ("P", 1)
    J1_expr = -ufl.grad(c1) - c1 * ufl.grad(phi) - (c1/(1-c1-c2))*(ufl.grad(c1)+ ufl.grad(c2))
    J1_func = fem.Function(W)
    J1_interpolated = fem.Expression(J1_expr, W.element.interpolation_points())
    J1_func.interpolate(J1_interpolated)
    j1_local = J1_func.eval(points_gathered[0], cells_gathered[0])
    
    J2_expr = -ufl.grad(c2) + c2 * ufl.grad(phi) - (c2/(1-c1-c2))*(ufl.grad(c1)+ ufl.grad(c2))
    J2_func = fem.Function(W)
    J2_interpolated = fem.Expression(J2_expr, W.element.interpolation_points())
    J2_func.interpolate(J2_interpolated)
    j2_local = J2_func.eval(points_gathered[0], cells_gathered[0])

    # J1 = ufl.div(-ufl.grad(c1) - c1 * ufl.grad(phi) - (c1/(1-c1-c2))*(ufl.grad(c1)+ ufl.grad(c2)))
    ...
    # J2 = ufl.div(-ufl.grad(c2) + c2 * ufl.grad(phi) - (c2/(1-c1-c2))*(ufl.grad(c1)+ ufl.grad(c2)))
    ...
    if MPI.COMM_WORLD.rank == 0:  
        ...
        if plot_flag:
            ...
            ax3.grid()
            ax3.legend()
            ax4.plot(points_combined[:, 0], j1_combined+j2_combined, color="green", linestyle="dashed", linewidth=2, label="jtot full")
            ax4.plot(points_combined[:, 0], c1_combined+c2_combined, color="green", linestyle="solid", linewidth=2, label="ctot full")
            ax4.plot(points_combined[:, 0], c1_combined-c2_combined, color="purple", linestyle="solid", linewidth=2, label="charge full")
    ...
    problem_SS = NonlinearPDE_SNESProblem(F_NSS, u, bcs)
    ...
    W = fem.functionspace(domain, ("CG", 1))  # o ("P", 1)
    J1_expr = -ufl.grad(c1) - c1 * ufl.grad(phi) +c1*(c1-c2)*ufl.grad(phi)
    J1_func = fem.Function(W)
    J1_interpolated = fem.Expression(J1_expr, W.element.interpolation_points())
    J1_func.interpolate(J1_interpolated)
    j1_local = J1_func.eval(points_gathered[0], cells_gathered[0])
    
    J2_expr = -ufl.grad(c2) +c2*ufl.grad(phi) +c2*(c1-c2)*ufl.grad(phi)
    J2_func = fem.Function(W)
    J2_interpolated = fem.Expression(J2_expr, W.element.interpolation_points())
    J2_func.interpolate(J2_interpolated)
    j2_local = J2_func.eval(points_gathered[0], cells_gathered[0])
    ...
    # J1 = ufl.div(-ufl.grad(c1) - c1 * ufl.grad(phi) +c1*(c1-c2)*ufl.grad(phi))
    # expr_J1 = fem.Expression(J1, points)
    # j1_local = expr_J1.eval(domain,cells_local)#*J1_char*(x_char**2)

    # # J2 = -ufl.grad(c2) + c2 * ufl.grad(phi) - (c2/(1-c1-c2))*(ufl.grad(c1)+ ufl.grad(c2))
    # J2 = ufl.div(-ufl.grad(c2) +c2*ufl.grad(phi) +c2*(c1-c2)*ufl.grad(phi))
    # expr_J2 = fem.Expression(J2,points)
    # j2_local = expr_J2.eval(domain,cells_local)#*J2_char*(x_char**2)
    ...
    if MPI.COMM_WORLD.rank == 0:  
        ...
        if plot_flag:
            ...
            # fig, ((ax1, ax2), (ax3, ax4)) =plt.subplots(2,2, figsize=(10,10))
            ax3.plot(points_combined[:, 0], phi_combined, color="orange", linestyle = "None", marker="D", markevery=500, label="H PB")
            ax3.legend()
            ax4.plot(points_combined[:, 0], j1_combined+j2_combined, color="green", linestyle="None", marker="v", markevery=500, label="jtot PB")
            ax4.plot(points_combined[:, 0], c1_combined+c2_combined, color="green", linestyle="None", marker="o", markevery=500, label="ctot PB")
            ax4.plot(points_combined[:, 0], c1_combined-c2_combined, color="purple", linestyle="None", marker="o", markevery=500, label="charge PB")

            ax4.legend()
            ax1.plot(points_combined[:, 0], j1_combined, color="red", linestyle="None", marker="v", markevery=500, label="j1 PB")
            ax1.plot(points_combined[:,0], c1_combined, color="red", linestyle="None", marker="o", markevery=500, label="c1 PB")
            ax1.legend()
            ax2.plot(points_combined[:, 0], j2_combined, color="blue", linestyle="None", marker="v", markevery=500, label="j2 PB")
            ax2.plot(points_combined[:,0], c2_combined, color="blue", linestyle="None", marker="o", markevery=500, label="c2 PB")
            ax2.legend()
            fig.savefig("esempio_Fenics.png", dpi=300, bbox_inches="tight")
            print("Done pt 2")  
            plt.show()
...
if __name__ == "__main__":
    ...
    # MEMBRANE BETWEEN TWO ELECTROLYTES 
    # 0      C     B      A
    # |------|MMMMM|------|
    ...
    imposed_L2=bulk_L
    imposed_R1=250
    imposed_R2=bulk_R*bulk_R/imposed_R1
    VA = VB+np.log(imposed_R1/bulk_R)
    print("We are applying "+str(VA) + " V")
    print("We are imposing at right c1= "+str(imposed_R1))
    print("We are imposing at right c2= "+str(imposed_R2))
