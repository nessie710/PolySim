...
model = gmsh.model()
central_electrode, bulk_marker, activated_electrodes, counter_electrodes, wall_marker = gmsh_5x5_cylinder(model, "Chamber", L_scaled, R_sens_scaled,R_cylinder_scaled,L_cylinder_scaled,dz_scaled,pitch_scaled)#partitioner = mesh.create_cell_partitioner(mesh.GhostMode.shared_facet)
partitioner = mesh.create_cell_partitioner(partitioner_parmetis())

domain, cell_tags, facet_tags = gmshio.read_from_msh("mesh3D_5x5.msh", MPI.COMM_WORLD,0 , gdim=3, partitioner=partitioner)
...
for (idx, omega) in enumerate(omegas):
    ...
    bcs = assemble_boundary_conditions_AC_electrode_matrix(VAC, facet_tags, fdim, V_bulk_scaled, Vapp_AC_scaled, c_bulk_scaled, bulk_marker, activated_electrodes, counter_electrodes, wall_marker)
    ...
    form = fem.form(J1_func[2]*ds(central_electrode))
    ...
    form = fem.form(J2_func[2]*ds(central_electrode))
    ...
    form = fem.form(field_func[2]*ds(central_electrode))
    ...
    area = fem.assemble_scalar(fem.form(1 * ds(central_electrode)))*x_char**2
...
if MPI.COMM_WORLD.rank == 0:  
    ...
    fig.savefig("capacitance_AC.png",bbox_inches="tight",dpi=300)
    #plt.show()
